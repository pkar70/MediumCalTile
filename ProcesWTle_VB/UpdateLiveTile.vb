...
Public NotInheritable Class UpdateLiveTile
    ...
    Public Async Sub ResetTriggers()
        ...
        If oBAS = BackgroundAccessStatus.AlwaysAllowed Or oBAS = BackgroundAccessStatus.AllowedSubjectToSystemPolicy Then
            ...
            Dim oTrig As SystemTrigger = New SystemTrigger(SystemTriggerType.ServicingComplete, True)
    ...
    Private Shared Function SafeString(sTxt As String) As String
        Dim sTmp As String = sTxt
        sTmp = sTmp.Replace("&", "&amp;")   ' to musi byc pierwsze!
        ' dopiero teraz pozostale
        sTmp = sTmp.Replace("<", "&gt;")
        sTmp = sTmp.Replace(">", "&lt;")
        sTmp = sTmp.Replace(vbCr, " ")   ' 20171114: bo miałem subject z <cr> i zepsuło to układ na Tile
    ...
    Private Async Function Calendar2TileAsync() As Task
        ...
        Dim oStore As AppointmentStore = Nothing
        ...
        End If
        If oStore Is Nothing Then Exit Function
        ...
        oCalOpt.FetchProperties.Add(Appointments.AppointmentProperties.StartTime)   ' default
        oCalOpt.FetchProperties.Add(Appointments.AppointmentProperties.Duration)
        oCalOpt.FetchProperties.Add(Appointments.AppointmentProperties.Subject)
        'If GetSettingsBool("convertHtmlTxt", False) Then
        ...
        If oCalendars Is Nothing Then Exit Function
        ...
        For Each oApp In oCalendars
            ...
            If iCnt = 0 Then
                ...
                If sOldEvent = sEvents1 Then Continue For
            ...
            Else
                ...
                If sOldEvent = sCurr Then Continue For
                sOldEvent = sCurr
                ' If oApp.AllDay Then
        ...
        If oTile IsNot Nothing Then TileUpdateManager.CreateTileUpdaterForApplication().Update(oTile)
